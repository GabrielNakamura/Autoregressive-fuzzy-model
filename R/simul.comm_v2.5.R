simul.comm<-function(Ncomm,Nspp,envir=FALSE,u=5,power=5,binary=FALSE,runs=30){
  pb<-txtProgressBar(min=0,max=runs,style=3)
  Res<-matrix(NA,nrow=runs,ncol=10,dimnames=list(1:runs,paste(c("R2.W","Coef.a.W","Coef.b.W","Beta.W","Value.PCPS.1","Value.PCPS.2","R2.Res","Coef.a.Res","Coef.b.Res","Beta.Res"))))
  tree.list<-list()
  ...
  resid.L.list<-list()
    for(k in 1:runs){
      ...
      N[,k]<-scales::rescale(niche,c(-1,101))
	    ...
	    pcps.L<-PCPS::pcps(L,cophenetic(tree),method="euclidean")
	    P<-pcps.L$P
	    P.list[[k]]<-P
	    L.list[[k]]<-L
	      for (c in 1:nrow(P)){
